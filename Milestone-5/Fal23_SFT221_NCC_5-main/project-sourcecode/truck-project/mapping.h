...
* @param backpath - the previous route we visited on the path we travelled so we will exclude travelling backwards.
* @returns - a collection of adjacent points we could potentially move to.
*/
struct Route getPossibleMoves(const struct Map* map, const struct Point p1, const struct Route r);
...
/*
* Check if point exist in route already
* @param r - the route
* @param p - the point
* @returns - true if p exists in r
*/
int ptExistInRt(const struct Route r, const struct Point p);

// As shortest path algorithm may find a path with redundant path because of obstacles, 
// this function is for finding out the redundant path and remove them
struct Route finalizeRoute(const struct Route r);

// calculate the direction by comparing the distance between to Point, stored in point
struct Point getDirection(const struct Point p1, const struct Point p2);

// check if the direction variable indicate it is a neighbour node
// dir Point contains the distance between Points
